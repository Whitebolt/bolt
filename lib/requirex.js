...
function boltRequireXLoader(bolt, boltLoaded) {
	...
	const boltHandlers = {
		...
		evaluated({target, exports, duration}) {
			if (isInternalModule(target) && fileCache.has(target) && !bolt.annotation.has(exports)) {
				const data = fileCache.get(target).toString('utf-8');
				fileCache.delete(target);
				bolt.annotation.from(wrapModuleInFunction(data), exports);
			}

			const moduleLoadTime = (((duration[0] * 1000000000) + duration[1]) / 1000000);
			const size = sizeBytesKMbGb(bolt.__moduleSizes.get(target) || 0);
			if (moduleLoadTime < 350) return bolt.emit('loadedModule', target, moduleLoadTime, size);
			if (moduleLoadTime > 1000) return bolt.emit('verySlowLoadedModule', target, moduleLoadTime, size);
			return bolt.emit('slowLoadedModule', target, moduleLoadTime, size);
		}
	};
